...
function fuzzyCandidateStarts(
  ...
  // `Array.from` rather than a spread: this module is mirrored into
  // `apps/web/lib/reading/` verbatim so the browser can resolve highlights
  // too, and that project compiles at an ES5 target where spreading a Set
  // does not type-check. Same array, no target assumption.
  return Array.from(starts).slice(0, MAX_FUZZY_CANDIDATES);
}
