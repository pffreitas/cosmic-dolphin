...
func NewOpenAILLM(apiKey string) *OpenAILLM {
	...
	return &OpenAILLM{client: client, toolCallMgr: NewToolCallManager()}
}
...
func NewOpenAILLMWithHost(apiKey string, host string) *OpenAILLM {
	...
	return &OpenAILLM{client: openAIClient, toolCallMgr: NewToolCallManager()}
}
...
func convertFromOpenAIToolCalls(toolCalls []openai.ToolCall) []LLMToolCall {
	if len(toolCalls) == 0 {
		return nil
	}

	calls := make([]LLMToolCall, len(toolCalls))
...
func (w *openAIStreamWrapper) Recv() (ChatCompletionResponse, error) {
	...
	for i, c := range resp.Choices {
		...
		if len(c.Delta.ToolCalls) > 0 {
			message.ToolCalls = make([]LLMToolCall, 0)
